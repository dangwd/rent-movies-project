    ...
    <section class="mt-9">
        <div class="mt-4 grid grid-cols-2 gap-y-5 sm:grid-cols-3 gap-x-5">
            <div v-for="(item, index) in Movies" :key="index">
                <router-link :to="`/detail/${item._id}`" class="flex flex-col rounded-xl overflow-hidden aspect-square border dark:border-zinc-600">
                    <img :src="item.images ? item.images[0] : ``" class="h-4/5 object-cover w-full" alt="" />
                    <div class="w-full h-1/5 bg-white dark:bg-zinc-800 dark:text-white px-3 flex items-center justify-between border-t-2 border-t-primary">
                        <span class="capitalize font-medium truncate">{{ item.movieName }}</span>
